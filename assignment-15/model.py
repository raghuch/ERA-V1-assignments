...
class LayerNormalization(nn.Module):
    def __init__(self, eps: float = 10**-6) -> None:
        ...
        self.bias = nn.Parameter(torch.zeros(1))  # Learnable


    def forward(self, x):
        # x: (batch, seq_len, hidden_sz)
...
class InputEmbeddings(nn.Module):
    def __init__(self, d_model: int, vocab_sz: int) -> None:
        super().__init__()
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_k = d_model // h # dimension seen by each head
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        # return attention scores also, which can be used for visualization
    ...
    def forward(self, q, k, v, mask):
        ...
        # x = x.transpose(1, 2).contiguous() # (batch, h, seq_len, d_k) --> (batch, seq_len, h, d_k)
        # x = x.view(x.shape[0], x.shape[1], self.d_model) # (batch, seq_len, h, d_k) --> (batch, seq_len, d_model)
...
class EncoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardNetwork, dropout: float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock,
                 feed_forward_block: FeedForwardNetwork, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, 
    ...
    print("building positional encoding")
